...
Большие языковые модели (LLM) \[9\] получают на вход документ и запрос на естественном языке и выдают ответ в произвольной или структурированной форме. Сильная сторона — *гибкость*: одна модель обрабатывает документы разных типов без специального обучения. Слабые стороны существенны: *нестабильность результатов* при повторных запросах; *ограниченная интерпретируемость* — причина ответа не поддаётся анализу; *риск конфабуляции* — генерации правдоподобных, но ошибочных значений; *зависимость от внешнего сервиса* и *стоимость обращений*. Эти ограничения делают неприемлемым применение языковой модели как единственного источника знаний в задаче пополнения онтологии: включение в формализованную модель сведений, не поддающихся проверке, противоречит цели формализации.

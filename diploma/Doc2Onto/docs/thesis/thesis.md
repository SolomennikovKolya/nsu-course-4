...
1. Проанализированы 4 категории готовых решений (Apache Tika, GROBID, облачные IDP-сервисы Google / Azure / Amazon, исследовательские системы Karma и KnowledgeStore); обоснована необходимость собственной разработки.
2. Сформулированы 11 функциональных и 6 нефункциональных требований, приняты 4 проектных ограничения.
3. Введена унифицированная модель представления документов из трёх типов блоков, формализованная XSD-схемой.
4. Разработан конвейер обработки документа из 5 стадий с возобновляемостью и контролируемым ручным вмешательством.
...
6. Разработан декларативный язык извлечения значений полей и реализована гибридная экстракция «детерминированный шаблон + коррекция большой языковой моделью».
7. Реализована концептно-ориентированная модель идентификации индивидов: 14 концептов, 4 стратегии формирования идентификаторов; для ФИО — морфологическое приведение к именительному падежу через pymorphy3.
8. Реализована двухслойная модель редактирования графа «черновик + правки» с сохранением правок между прогонами шаблона.
...
12. Реализована подсистема взаимодействия с большой языковой моделью (OpenAI SDK, `gpt-5.4-mini`) с тремя точками применения и полным журналированием обращений.

Программная часть — более 12 тысяч строк собственного исходного кода на языке Python (без учёта пустых строк и комментариев). Качественная апробация проведена на 15 реальных документах кафедры; для всех документов конвейер достиг последней стадии без ручного вмешательства, кросс-форматный тест DOCX / DOC / PDF одного документа дал идентичный итоговый граф фактов. Для работы составлены 6 приложений (А — XSD-схема UDDM, Б — пример UDDM, В — фрагмент онтологии, Г — пример кода шаблона, Д — описание программы, Е — руководство оператора).
...
27. Соломенников Н. А. Doc2Onto: настольное приложение для автоматизированного пополнения онтологической модели кафедры знаниями, извлечёнными из документов // Материалы 64-й Международной научной студенческой конференции. — Новосибирск, 15–21 апреля 2026 г. — Новосибирск : ИПЦ НГУ, 2026.

...
# Хелперы билдера прячут за собой типы, литералы и форму IRI — пользователю
# (и LLM-агенту) НЕ нужно вручную писать ValueTransformer / лямбды для этих
# случаев. Зашиты здесь, потому что в TBox эта информация отсутствует.
_IRI_GUIDE: Dict[str, str] = {
    "Персона": 'b.person("<поле_фио>", role=ONTO.Персона) — IRI = :Персона_<sha1>, литералы :фио/:фамилия/:имя/:отчество и тип добавляются автоматически',
    "Студент": 'b.person("<поле_фио>", role=ONTO.Студент) — то же, плюс тип :Студент',
    "Сотрудник": 'b.person("<поле_фио>", role=ONTO.Сотрудник) — то же, плюс тип :Сотрудник',
    "Организация": 'b.organization("<поле_названия>") — IRI = :Организация_<sha1>, литерал :названиеОрганизации',
    "Университет": 'b.organization("<поле_названия>", role=ONTO.Университет)',
    "Факультет": 'b.organization("<поле_названия>", role=ONTO.Факультет)',
    "Кафедра": 'b.organization("<поле_названия>", role=ONTO.Кафедра)',
    "Лаборатория": 'b.organization("<поле_названия>", role=ONTO.Лаборатория)',
    "ВнешняяОрганизация": 'b.organization("<поле_названия>", role=ONTO.ВнешняяОрганизация)',
    "СтруктурноеПодразделение": 'b.organization("<поле_названия>", role=ONTO.СтруктурноеПодразделение)',
    "Группа": 'b.group("<поле_номера>") — IRI = :Группа_<номер>, литерал :номерГруппы',
    ...
    "Профиль": 'b.profile("<поле_названия>") — IRI = :Профиль_<sha1>, литерал :названиеПрофиля',
    "ВКР": 'b.thesis(title_field="<поле_темы>", student=<student_iri>) — IRI = :ВКР_<sha1>, тип :ВКР, литерал :темаВКР, связь :авторВКР',
    "Практика": 'b.practice(student=<student_iri>, kind=<kind_iri>, year_field="<поле_года>") — IRI = :Практика_<sha1>',
    "Должность": 'b.position("<поле_должности>") — выбирает индивид перечисления :Должность_X',
    "УченаяСтепень": 'b.degree("<поле_степени>") — выбирает индивид перечисления :УченаяСтепень_X',
    "УченоеЗвание": 'b.title("<поле_звания>") — выбирает индивид перечисления :УченоеЗвание_X',
    "ВидПрактики": 'b.practice_kind("<поле_вида_практики>") — выбирает индивид перечисления :ВидПрактики_X',
    "ОценкаПрактики": 'b.grade("<поле_оценки>") — выбирает индивид перечисления :Оценка_X',
    "Агент": None,
